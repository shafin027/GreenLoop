...
export const AdminDashboard: React.FC = () => {
  ...
  const handleVerifyUser = async (id: string, verify: boolean) => {
    setActionInProgress(`verify-${id}`);
    try {
      const res = await fetch(`/api/admin/${verify ? 'verify-user' : 'unverify-user'}/${id}`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const d = await res.json();
      if (res.ok) { showMessage('success', d.message); fetchAllUsers(); }
      else showMessage('error', d.message || 'Failed');
    } catch (err) { showMessage('error', 'An error occurred'); } finally { setActionInProgress(null); }
  };

  const handleVerifyBusiness = async (id: string, verify: boolean) => {
    setActionInProgress(`verify-${id}`);
    try {
      const res = await fetch(`/api/admin/${verify ? 'verify-business' : 'unverify-business'}/${id}`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const d = await res.json();
      if (res.ok) { showMessage('success', d.message); fetchAllBusinesses(); }
      else showMessage('error', d.message || 'Failed');
    } catch (err) { showMessage('error', 'An error occurred'); } finally { setActionInProgress(null); }
  };
  ...
  const fetchCenters = async () => {
    if (!token) return;
    try {
      const res = await fetch('/api/admin/recycling-centers', { headers: { 'Authorization': `Bearer ${token}` } });
      const data = await res.json();
  ...
  const fetchAdminPosts = async () => {
    try {
      const res = await fetch('/api/community/admin/posts', { headers: { 'Authorization': `Bearer ${token}` } });
      const data = await res.json();
...
const handleDeleteEvent = async (id: string) => {
    ...
    setDeletingId(id);
    try {
      const res = await fetch(`/api/community/admin/events/${id}`, {
...
const handleSaveEvent = async () => {
    ...
    try {
      const res = await fetch(`/api/community/admin/events/${editingEvent._id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
  ...
  const handleToggleApproval = async (post: any) => {
    try {
      const res = await fetch(`/api/community/admin/posts/${post._id || post.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
  ...
  const handleSavePost = async () => {
    ...
    try {
      const res = await fetch(`/api/community/admin/posts/${editingPost._id || editingPost.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
  ...
  const handleVerifyCollector = async (id: string, verify: boolean) => {
    ...
    try {
      ...
      const d = await res.json();
      if (res.ok) { 
        showMessage('success', d.message || `Collector ${verify ? 'verified' : 'unverified'}!`); 
        fetchCollectors(); 
      } else {
        showMessage('error', d.message || 'Failed to update collector status');
      }
    } catch (err) { 
      console.error(err); 
                    ...
                    <tr key={c._id} className={`hover:bg-white/5 transition-colors ${c.isBanned ? 'opacity-60' : ''}`}>
                      ...
                      <td className="px-6 py-5">
                        <div className="flex items-center gap-3">
                          <button onClick={() => handleVerifyCollector(c._id, !c.verified)} disabled={actionInProgress === `verify-${c._id}` || !!actionInProgress}
                            ...
                            {actionInProgress === `verify-${c._id}` ? '...' : c.verified ? 'Unverify' : 'Verify'}
                          </button>
                  ...
                  {allUsers.map((u: any) => (
                    <tr key={u._id} className={`hover:bg-white/5 transition-colors ${u.isBanned ? 'opacity-60' : ''}`}>
                      ...
                      <td className="px-6 py-5">
                        <div className="flex flex-wrap gap-1">
                          {u.verified
                            ? <span className="px-2 py-0.5 rounded-full text-xs font-bold bg-emerald-500/10 text-emerald-400">Verified</span>
                            : <span className="px-2 py-0.5 rounded-full text-xs font-bold bg-amber-500/10 text-amber-400">Pending</span>}
                          {u.isBanned
                      ...
                      <td className="px-6 py-5">
                        <div className="flex items-center gap-3">
                          <button onClick={() => handleVerifyUser(u._id, !u.verified)} disabled={actionInProgress === `verify-${u._id}`}
                            className={`text-xs font-bold uppercase tracking-widest disabled:opacity-40 ${u.verified ? 'text-amber-400 hover:text-amber-300' : 'text-emerald-400 hover:text-emerald-300'}`}>
                            {actionInProgress === `verify-${u._id}` ? '...' : u.verified ? 'Unverify' : 'Verify'}
                          </button>
                ...
                <tbody className="divide-y divide-white/5">
                  {allBusinesses.map((b: any) => (
                    <tr key={b._id} className={`hover:bg-white/5 transition-colors ${b.isBanned ? 'opacity-60' : ''}`}>
                      ...
                      <td className="px-6 py-5">
                        <div className="flex items-center gap-3">
                          <button onClick={() => handleVerifyBusiness(b._id, !b.verified)} disabled={actionInProgress === `verify-${b._id}`}
                            className={`text-xs font-bold uppercase tracking-widest disabled:opacity-40 ${b.verified ? 'text-amber-400 hover:text-amber-300' : 'text-emerald-400 hover:text-emerald-300'}`}>
                            {actionInProgress === `verify-${b._id}` ? '...' : b.verified ? 'Unverify' : 'Verify'}
                          </button>
